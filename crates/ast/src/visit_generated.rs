...
pub trait Pass<'alloc> {
    fn visit_argument(&mut self, ast: &mut Argument<'alloc>) {
        ...
        match ast {
            Argument::SpreadElement(ast) => self.visit_enum_argument_variant_spread_element(ast),
            Argument::Expression(ast) => self.visit_enum_argument_variant_expression(ast),
        }
    ...
    fn visit_binary_operator(&mut self, ast: &mut BinaryOperator) {
        ...
        match ast {
            BinaryOperator::Equals { .. } => self.visit_enum_binary_operator_variant_equals(),
            ...
            BinaryOperator::LessThan { .. } => self.visit_enum_binary_operator_variant_less_than(),
            ...
            BinaryOperator::In { .. } => self.visit_enum_binary_operator_variant_in(),
            ...
            BinaryOperator::Add { .. } => self.visit_enum_binary_operator_variant_add(),
            ...
            BinaryOperator::Div { .. } => self.visit_enum_binary_operator_variant_div(),
            BinaryOperator::Mod { .. } => self.visit_enum_binary_operator_variant_mod(),
            BinaryOperator::Pow { .. } => self.visit_enum_binary_operator_variant_pow(),
            BinaryOperator::Comma { .. } => self.visit_enum_binary_operator_variant_comma(),
            BinaryOperator::Coalesce { .. } => self.visit_enum_binary_operator_variant_coalesce(),
    ...
    fn visit_unary_operator(&mut self, ast: &mut UnaryOperator) {
        ...
        match ast {
            UnaryOperator::Plus { .. } => self.visit_enum_unary_operator_variant_plus(),
            UnaryOperator::Minus { .. } => self.visit_enum_unary_operator_variant_minus(),
            ...
            UnaryOperator::Typeof { .. } => self.visit_enum_unary_operator_variant_typeof(),
            UnaryOperator::Void { .. } => self.visit_enum_unary_operator_variant_void(),
            UnaryOperator::Delete { .. } => self.visit_enum_unary_operator_variant_delete(),
        }
    ...
    fn visit_update_operator(&mut self, ast: &mut UpdateOperator) {
        ...
        match ast {
            UpdateOperator::Increment { .. } => self.visit_enum_update_operator_variant_increment(),
            UpdateOperator::Decrement { .. } => self.visit_enum_update_operator_variant_decrement(),
        }
    ...
    fn visit_program(&mut self, ast: &mut Program<'alloc>) {
        ...
        match ast {
            Program::Module(ast) => self.visit_enum_program_variant_module(ast),
            Program::Script(ast) => self.visit_enum_program_variant_script(ast),
        }
    ...
    fn visit_enum_program_variant_module(&mut self, ast: &mut Module<'alloc>) {
        self.enter_enum_program_variant_module(ast);
        self.visit_module(ast);
        self.leave_enum_program_variant_module(ast);
    }

    fn enter_enum_program_variant_module(&mut self, ast: &mut Module<'alloc>) {}

    fn leave_enum_program_variant_module(&mut self, ast: &mut Module<'alloc>) {}

    fn visit_enum_program_variant_script(&mut self, ast: &mut Script<'alloc>) {
        self.enter_enum_program_variant_script(ast);
        self.visit_script(ast);
        self.leave_enum_program_variant_script(ast);
    }

    fn enter_enum_program_variant_script(&mut self, ast: &mut Script<'alloc>) {}

    fn leave_enum_program_variant_script(&mut self, ast: &mut Script<'alloc>) {}
    ...
    fn visit_statement(&mut self, ast: &mut Statement<'alloc>) {
        ...
        match ast {
            Statement::BlockStatement { block, .. } => {
                self.visit_enum_statement_variant_block_statement(block)
            }
            Statement::BreakStatement { label, .. } => {
                self.visit_enum_statement_variant_break_statement(label)
            }
            Statement::ContinueStatement { label, .. } => {
                self.visit_enum_statement_variant_continue_statement(label)
            }
            Statement::DebuggerStatement { .. } => {
                self.visit_enum_statement_variant_debugger_statement()
            }
            Statement::DoWhileStatement { block, test, .. } => {
                self.visit_enum_statement_variant_do_while_statement(block, test)
            }
            Statement::EmptyStatement { .. } => self.visit_enum_statement_variant_empty_statement(),
            Statement::ExpressionStatement(ast) => {
                self.visit_enum_statement_variant_expression_statement(ast)
            }
            Statement::ForInStatement {
                left, right, block, ..
            } => self.visit_enum_statement_variant_for_in_statement(left, right, block),
            Statement::ForOfStatement {
                left, right, block, ..
            } => self.visit_enum_statement_variant_for_of_statement(left, right, block),
            Statement::ForStatement {
                ...
                block,
                ..
            } => self.visit_enum_statement_variant_for_statement(init, test, update, block),
            Statement::IfStatement(ast) => self.visit_enum_statement_variant_if_statement(ast),
            Statement::LabeledStatement { label, body, .. } => {
                self.visit_enum_statement_variant_labeled_statement(label, body)
            }
            Statement::ReturnStatement { expression, .. } => {
                self.visit_enum_statement_variant_return_statement(expression)
            }
            Statement::SwitchStatement {
                discriminant,
                cases,
                ..
            } => self.visit_enum_statement_variant_switch_statement(discriminant, cases),
            ...
            Statement::ThrowStatement { expression, .. } => {
                self.visit_enum_statement_variant_throw_statement(expression)
            }
            Statement::TryCatchStatement {
                body, catch_clause, ..
            } => self.visit_enum_statement_variant_try_catch_statement(body, catch_clause),
            ...
            Statement::WhileStatement { test, block, .. } => {
                self.visit_enum_statement_variant_while_statement(test, block)
            }
            Statement::WithStatement { object, body, .. } => {
                self.visit_enum_statement_variant_with_statement(object, body)
            }
    ...
    fn visit_enum_statement_variant_block_statement(&mut self, block: &mut Block<'alloc>) {
        self.enter_enum_statement_variant_block_statement(block);
        self.visit_block(block);
        self.leave_enum_statement_variant_block_statement(block);
    }

    fn enter_enum_statement_variant_block_statement(&mut self, block: &mut Block<'alloc>) {}

    fn leave_enum_statement_variant_block_statement(&mut self, block: &mut Block<'alloc>) {}

    fn visit_enum_statement_variant_break_statement(&mut self, label: &mut Option<Label>) {
        self.enter_enum_statement_variant_break_statement(label);
        if let Some(item) = label {
            self.visit_label(item);
        }
        self.leave_enum_statement_variant_break_statement(label);
    }

    fn enter_enum_statement_variant_break_statement(&mut self, label: &mut Option<Label>) {}

    fn leave_enum_statement_variant_break_statement(&mut self, label: &mut Option<Label>) {}

    fn visit_enum_statement_variant_continue_statement(&mut self, label: &mut Option<Label>) {
        self.enter_enum_statement_variant_continue_statement(label);
        if let Some(item) = label {
            self.visit_label(item);
        }
        self.leave_enum_statement_variant_continue_statement(label);
    }

    fn enter_enum_statement_variant_continue_statement(&mut self, label: &mut Option<Label>) {}

    fn leave_enum_statement_variant_continue_statement(&mut self, label: &mut Option<Label>) {}
    ...
    fn visit_enum_statement_variant_do_while_statement(
        ...
        self.enter_enum_statement_variant_do_while_statement(block, test);
        self.visit_statement(block);
        self.visit_expression(test);
        self.leave_enum_statement_variant_do_while_statement(block, test);
    }
    ...
    fn visit_enum_statement_variant_for_in_statement(
        ...
        self.enter_enum_statement_variant_for_in_statement(left, right, block);
        self.visit_variable_declaration_or_assignment_target(left);
        self.visit_expression(right);
        self.visit_statement(block);
        self.leave_enum_statement_variant_for_in_statement(left, right, block);
    }
    ...
    fn visit_enum_statement_variant_for_of_statement(
        ...
        self.enter_enum_statement_variant_for_of_statement(left, right, block);
        self.visit_variable_declaration_or_assignment_target(left);
        self.visit_expression(right);
        self.visit_statement(block);
        self.leave_enum_statement_variant_for_of_statement(left, right, block);
    }
    ...
    fn visit_enum_statement_variant_for_statement(
        ...
        self.enter_enum_statement_variant_for_statement(init, test, update, block);
        ...
        self.visit_statement(block);
        self.leave_enum_statement_variant_for_statement(init, test, update, block);
    }
    ...
    fn visit_enum_statement_variant_if_statement(&mut self, ast: &mut IfStatement<'alloc>) {
    ...
    fn enter_enum_statement_variant_if_statement(&mut self, ast: &mut IfStatement<'alloc>) {}

    fn leave_enum_statement_variant_if_statement(&mut self, ast: &mut IfStatement<'alloc>) {}
    ...
    fn visit_enum_statement_variant_labeled_statement(
        ...
        self.enter_enum_statement_variant_labeled_statement(label, body);
        self.visit_label(label);
        self.visit_statement(body);
        self.leave_enum_statement_variant_labeled_statement(label, body);
    }
    ...
    fn visit_enum_statement_variant_return_statement(
        ...
        self.enter_enum_statement_variant_return_statement(expression);
        if let Some(item) = expression {
            self.visit_expression(item);
        }
        self.leave_enum_statement_variant_return_statement(expression);
    }
    ...
    fn visit_enum_statement_variant_switch_statement(
        ...
        self.enter_enum_statement_variant_switch_statement(discriminant, cases);
        self.visit_expression(discriminant);
        for item in cases.iter_mut() {
            self.visit_switch_case(item);
        }
        self.leave_enum_statement_variant_switch_statement(discriminant, cases);
    }
    ...
    fn visit_enum_statement_variant_throw_statement(
        ...
        self.enter_enum_statement_variant_throw_statement(expression);
        self.visit_expression(expression);
        self.leave_enum_statement_variant_throw_statement(expression);
    }
    ...
    fn visit_enum_statement_variant_try_catch_statement(
        ...
        self.enter_enum_statement_variant_try_catch_statement(body, catch_clause);
        self.visit_block(body);
        self.visit_catch_clause(catch_clause);
        self.leave_enum_statement_variant_try_catch_statement(body, catch_clause);
    }
    ...
    fn visit_enum_statement_variant_try_finally_statement(
        ...
        self.enter_enum_statement_variant_try_finally_statement(body, catch_clause, finalizer);
        self.visit_block(body);
        if let Some(item) = catch_clause {
            self.visit_catch_clause(item);
        }
        self.visit_block(finalizer);
        self.leave_enum_statement_variant_try_finally_statement(body, catch_clause, finalizer);
    }
    ...
    fn visit_enum_statement_variant_while_statement(
        ...
        self.enter_enum_statement_variant_while_statement(test, block);
        self.visit_expression(test);
        self.visit_statement(block);
        self.leave_enum_statement_variant_while_statement(test, block);
    }
    ...
    fn visit_enum_statement_variant_with_statement(
        ...
        self.enter_enum_statement_variant_with_statement(object, body);
        self.visit_expression(object);
        self.visit_statement(body);
        self.leave_enum_statement_variant_with_statement(object, body);
    }
    ...
    fn visit_enum_statement_variant_function_declaration(&mut self, ast: &mut Function<'alloc>) {
    ...
    fn leave_enum_statement_variant_function_declaration(&mut self, ast: &mut Function<'alloc>) {}
    ...
    fn visit_expression(&mut self, ast: &mut Expression<'alloc>) {
        ...
        match ast {
            ...
            Expression::LiteralBooleanExpression { value, .. } => {
                self.visit_enum_expression_variant_literal_boolean_expression(value)
            }
            ...
            Expression::LiteralStringExpression { value, .. } => {
                self.visit_enum_expression_variant_literal_string_expression(value)
            }
            ...
            Expression::ArrowExpression {
                ...
                body,
                ..
            } => self.visit_enum_expression_variant_arrow_expression(is_async, params, body),
            Expression::AssignmentExpression {
                binding,
                expression,
                ..
            } => self.visit_enum_expression_variant_assignment_expression(binding, expression),
            Expression::BinaryExpression {
                operator,
                left,
                right,
                ..
            } => self.visit_enum_expression_variant_binary_expression(operator, left, right),
            ...
            Expression::ConditionalExpression {
                ...
                .visit_enum_expression_variant_conditional_expression(test, consequent, alternate),
            ...
            Expression::NewExpression {
                callee, arguments, ..
            } => self.visit_enum_expression_variant_new_expression(callee, arguments),
            ...
            Expression::OptionalExpression { object, tail, .. } => {
                self.visit_enum_expression_variant_optional_expression(object, tail)
            }
            ...
            Expression::YieldExpression { expression, .. } => {
                self.visit_enum_expression_variant_yield_expression(expression)
            }
            Expression::YieldGeneratorExpression { expression, .. } => {
                self.visit_enum_expression_variant_yield_generator_expression(expression)
            }
            Expression::AwaitExpression { expression, .. } => {
                self.visit_enum_expression_variant_await_expression(expression)
            }
            Expression::ImportCallExpression { argument, .. } => {
                self.visit_enum_expression_variant_import_call_expression(argument)
            }
        }
    ...
    fn visit_enum_expression_variant_literal_boolean_expression(&mut self, value: &mut bool) {
        self.enter_enum_expression_variant_literal_boolean_expression(value);
        self.leave_enum_expression_variant_literal_boolean_expression(value);
    }

    fn enter_enum_expression_variant_literal_boolean_expression(&mut self, value: &mut bool) {}

    fn leave_enum_expression_variant_literal_boolean_expression(&mut self, value: &mut bool) {}
    ...
    fn visit_enum_expression_variant_literal_string_expression(
        ...
        self.enter_enum_expression_variant_literal_string_expression(value);
        self.leave_enum_expression_variant_literal_string_expression(value);
    }
    ...
    fn visit_enum_expression_variant_arrow_expression(
        ...
        self.enter_enum_expression_variant_arrow_expression(is_async, params, body);
        self.visit_formal_parameters(params);
        self.visit_arrow_expression_body(body);
        self.leave_enum_expression_variant_arrow_expression(is_async, params, body);
    }
    ...
    fn visit_enum_expression_variant_assignment_expression(
        ...
        self.enter_enum_expression_variant_assignment_expression(binding, expression);
        self.visit_assignment_target(binding);
        self.visit_expression(expression);
        self.leave_enum_expression_variant_assignment_expression(binding, expression);
    }
    ...
    fn visit_enum_expression_variant_binary_expression(
        ...
        self.enter_enum_expression_variant_binary_expression(operator, left, right);
        self.visit_binary_operator(operator);
        self.visit_expression(left);
        self.visit_expression(right);
        self.leave_enum_expression_variant_binary_expression(operator, left, right);
    }
    ...
    fn visit_enum_expression_variant_call_expression(&mut self, ast: &mut CallExpression<'alloc>) {
    ...
    fn enter_enum_expression_variant_call_expression(&mut self, ast: &mut CallExpression<'alloc>) {}

    fn leave_enum_expression_variant_call_expression(&mut self, ast: &mut CallExpression<'alloc>) {}
    ...
    fn visit_enum_expression_variant_conditional_expression(
        ...
        self.enter_enum_expression_variant_conditional_expression(test, consequent, alternate);
        self.visit_expression(test);
        self.visit_expression(consequent);
        self.visit_expression(alternate);
        self.leave_enum_expression_variant_conditional_expression(test, consequent, alternate);
    }
    ...
    fn visit_enum_expression_variant_function_expression(&mut self, ast: &mut Function<'alloc>) {
        self.enter_enum_expression_variant_function_expression(ast);
        self.visit_function(ast);
        self.leave_enum_expression_variant_function_expression(ast);
    }

    fn enter_enum_expression_variant_function_expression(&mut self, ast: &mut Function<'alloc>) {}

    fn leave_enum_expression_variant_function_expression(&mut self, ast: &mut Function<'alloc>) {}
    ...
    fn visit_enum_expression_variant_new_expression(
        ...
        self.enter_enum_expression_variant_new_expression(callee, arguments);
        self.visit_expression(callee);
        self.visit_arguments(arguments);
        self.leave_enum_expression_variant_new_expression(callee, arguments);
    }
    ...
    fn visit_enum_expression_variant_optional_expression(
        ...
        self.enter_enum_expression_variant_optional_expression(object, tail);
        self.visit_expression_or_super(object);
        self.visit_expression(tail);
        self.leave_enum_expression_variant_optional_expression(object, tail);
    }
    ...
    fn visit_enum_expression_variant_optional_chain(&mut self, ast: &mut OptionalChain<'alloc>) {
    ...
    fn enter_enum_expression_variant_optional_chain(&mut self, ast: &mut OptionalChain<'alloc>) {}

    fn leave_enum_expression_variant_optional_chain(&mut self, ast: &mut OptionalChain<'alloc>) {}
    ...
    fn visit_enum_expression_variant_unary_expression(
        ...
        self.enter_enum_expression_variant_unary_expression(operator, operand);
        self.visit_unary_operator(operator);
        self.visit_expression(operand);
        self.leave_enum_expression_variant_unary_expression(operator, operand);
    }
    ...
    fn visit_enum_expression_variant_update_expression(
        ...
        self.enter_enum_expression_variant_update_expression(is_prefix, operator, operand);
    ...
    fn visit_enum_expression_variant_yield_expression(
        ...
        self.enter_enum_expression_variant_yield_expression(expression);
        if let Some(item) = expression {
            self.visit_expression(item);
        }
        self.leave_enum_expression_variant_yield_expression(expression);
    }
    ...
    fn visit_enum_expression_variant_yield_generator_expression(
        ...
        self.enter_enum_expression_variant_yield_generator_expression(expression);
        self.visit_expression(expression);
        self.leave_enum_expression_variant_yield_generator_expression(expression);
    }
    ...
    fn visit_enum_expression_variant_await_expression(
        ...
        self.enter_enum_expression_variant_await_expression(expression);
        self.visit_expression(expression);
        self.leave_enum_expression_variant_await_expression(expression);
    }
    ...
    fn visit_enum_expression_variant_import_call_expression(
        ...
        self.enter_enum_expression_variant_import_call_expression(argument);
        self.visit_expression(argument);
        self.leave_enum_expression_variant_import_call_expression(argument);
    }
    ...
    fn visit_optional_chain(&mut self, ast: &mut OptionalChain<'alloc>) {
        ...
        match ast {
            OptionalChain::ComputedMemberExpressionTail { expression, .. } => {
                self.visit_enum_optional_chain_variant_computed_member_expression_tail(expression)
            }
            OptionalChain::StaticMemberExpressionTail { property, .. } => {
                self.visit_enum_optional_chain_variant_static_member_expression_tail(property)
            }
            OptionalChain::CallExpressionTail { arguments, .. } => {
                self.visit_enum_optional_chain_variant_call_expression_tail(arguments)
            }
    ...
    fn visit_enum_optional_chain_variant_computed_member_expression_tail(
        ...
        self.enter_enum_optional_chain_variant_computed_member_expression_tail(expression);
        self.visit_expression(expression);
        self.leave_enum_optional_chain_variant_computed_member_expression_tail(expression);
    }
    ...
    fn visit_enum_optional_chain_variant_static_member_expression_tail(
        ...
        self.enter_enum_optional_chain_variant_static_member_expression_tail(property);
        self.visit_identifier_name(property);
        self.leave_enum_optional_chain_variant_static_member_expression_tail(property);
    }
    ...
    fn visit_enum_optional_chain_variant_call_expression_tail(
        ...
        self.enter_enum_optional_chain_variant_call_expression_tail(arguments);
        self.visit_arguments(arguments);
        self.leave_enum_optional_chain_variant_call_expression_tail(arguments);
    }
    ...
    fn visit_method_definition(&mut self, ast: &mut MethodDefinition<'alloc>) {
        ...
        match ast {
            MethodDefinition::Method(ast) => self.visit_enum_method_definition_variant_method(ast),
            MethodDefinition::Getter(ast) => self.visit_enum_method_definition_variant_getter(ast),
            MethodDefinition::Setter(ast) => self.visit_enum_method_definition_variant_setter(ast),
        }
    ...
    fn visit_enum_method_definition_variant_method(&mut self, ast: &mut Method<'alloc>) {
    ...
    fn enter_enum_method_definition_variant_method(&mut self, ast: &mut Method<'alloc>) {}

    fn leave_enum_method_definition_variant_method(&mut self, ast: &mut Method<'alloc>) {}

    fn visit_enum_method_definition_variant_getter(&mut self, ast: &mut Getter<'alloc>) {
    ...
    fn enter_enum_method_definition_variant_getter(&mut self, ast: &mut Getter<'alloc>) {}

    fn leave_enum_method_definition_variant_getter(&mut self, ast: &mut Getter<'alloc>) {}

    fn visit_enum_method_definition_variant_setter(&mut self, ast: &mut Setter<'alloc>) {
    ...
    fn enter_enum_method_definition_variant_setter(&mut self, ast: &mut Setter<'alloc>) {}

    fn leave_enum_method_definition_variant_setter(&mut self, ast: &mut Setter<'alloc>) {}
    ...
    fn visit_enum_import_declaration_variant_import(&mut self, ast: &mut Import<'alloc>) {
    ...
    fn enter_enum_import_declaration_variant_import(&mut self, ast: &mut Import<'alloc>) {}

    fn leave_enum_import_declaration_variant_import(&mut self, ast: &mut Import<'alloc>) {}
    ...
    fn visit_enum_export_declaration_variant_export_all_from(&mut self, ast: &mut ExportAllFrom) {
    ...
    fn enter_enum_export_declaration_variant_export_all_from(&mut self, ast: &mut ExportAllFrom) {}

    fn leave_enum_export_declaration_variant_export_all_from(&mut self, ast: &mut ExportAllFrom) {}

    fn visit_enum_export_declaration_variant_export_from(&mut self, ast: &mut ExportFrom<'alloc>) {
    ...
    fn enter_enum_export_declaration_variant_export_from(&mut self, ast: &mut ExportFrom<'alloc>) {}

    fn leave_enum_export_declaration_variant_export_from(&mut self, ast: &mut ExportFrom<'alloc>) {}
    ...
    fn visit_enum_export_declaration_variant_export(&mut self, ast: &mut Export<'alloc>) {
    ...
    fn enter_enum_export_declaration_variant_export(&mut self, ast: &mut Export<'alloc>) {}

    fn leave_enum_export_declaration_variant_export(&mut self, ast: &mut Export<'alloc>) {}
    ...
    fn visit_enum_binding_pattern_variant_array_binding(&mut self, ast: &mut ArrayBinding<'alloc>) {
    ...
    fn enter_enum_binding_pattern_variant_array_binding(&mut self, ast: &mut ArrayBinding<'alloc>) {
    }

    fn leave_enum_binding_pattern_variant_array_binding(&mut self, ast: &mut ArrayBinding<'alloc>) {
    }

    fn visit_binding(&mut self, ast: &mut Binding<'alloc>) {
        self.enter_binding(ast);
        match ast {
            Binding::BindingPattern(ast) => self.visit_enum_binding_variant_binding_pattern(ast),
    ...
    fn visit_enum_binding_variant_binding_pattern(&mut self, ast: &mut BindingPattern<'alloc>) {
    ...
    fn enter_enum_binding_variant_binding_pattern(&mut self, ast: &mut BindingPattern<'alloc>) {}

    fn leave_enum_binding_variant_binding_pattern(&mut self, ast: &mut BindingPattern<'alloc>) {}

    fn visit_enum_binding_variant_binding_identifier(&mut self, ast: &mut BindingIdentifier) {
        self.enter_enum_binding_variant_binding_identifier(ast);
        self.visit_binding_identifier(ast);
        self.leave_enum_binding_variant_binding_identifier(ast);
    }

    fn enter_enum_binding_variant_binding_identifier(&mut self, ast: &mut BindingIdentifier) {}

    fn leave_enum_binding_variant_binding_identifier(&mut self, ast: &mut BindingIdentifier) {}
    ...
    fn visit_parameter(&mut self, ast: &mut Parameter<'alloc>) {
        ...
        match ast {
            Parameter::Binding(ast) => self.visit_enum_parameter_variant_binding(ast),
    ...
    fn visit_enum_parameter_variant_binding(&mut self, ast: &mut Binding<'alloc>) {
        self.enter_enum_parameter_variant_binding(ast);
        self.visit_binding(ast);
        self.leave_enum_parameter_variant_binding(ast);
    }

    fn enter_enum_parameter_variant_binding(&mut self, ast: &mut Binding<'alloc>) {}

    fn leave_enum_parameter_variant_binding(&mut self, ast: &mut Binding<'alloc>) {}
    ...
    fn visit_expression_or_super(&mut self, ast: &mut ExpressionOrSuper<'alloc>) {
        ...
        match ast {
            ...
            ExpressionOrSuper::Super { .. } => self.visit_enum_expression_or_super_variant_super(),
        }
    ...
    fn visit_member_assignment_target(&mut self, ast: &mut MemberAssignmentTarget<'alloc>) {
        ...
        match ast {
            MemberAssignmentTarget::ComputedMemberAssignmentTarget(ast) => self
                .visit_enum_member_assignment_target_variant_computed_member_assignment_target(ast),
            MemberAssignmentTarget::StaticMemberAssignmentTarget(ast) => self
                .visit_enum_member_assignment_target_variant_static_member_assignment_target(ast),
        }
    ...
    fn leave_computed_member_assignment_target(
        &mut self,
        ast: &mut ComputedMemberAssignmentTarget<'alloc>,
    ) {
    }
    ...
    fn leave_static_member_assignment_target(
        &mut self,
        ast: &mut StaticMemberAssignmentTarget<'alloc>,
    ) {
    }
    ...
    fn leave_assignment_target_with_default(
        &mut self,
        ast: &mut AssignmentTargetWithDefault<'alloc>,
    ) {
    }
    ...
    fn visit_assignment_target_maybe_default(
        ...
        match ast {
            ...
            AssignmentTargetMaybeDefault::AssignmentTargetWithDefault(ast) => self
    ...
    fn leave_assignment_target_maybe_default(
        &mut self,
        ast: &mut AssignmentTargetMaybeDefault<'alloc>,
    ) {
    }
    ...
    fn visit_enum_assignment_target_property_variant_assignment_target_property_identifier(
        ...
        self.enter_enum_assignment_target_property_variant_assignment_target_property_identifier(
            ast,
        );
        self.visit_assignment_target_property_identifier(ast);
        self.leave_enum_assignment_target_property_variant_assignment_target_property_identifier(
            ast,
        );
    }
    ...
    fn leave_assignment_target_property_identifier(
        &mut self,
        ast: &mut AssignmentTargetPropertyIdentifier<'alloc>,
    ) {
    }
    ...
    fn leave_assignment_target_property_property(
        &mut self,
        ast: &mut AssignmentTargetPropertyProperty<'alloc>,
    ) {
    }
    ...
    fn visit_class_element(&mut self, ast: &mut ClassElement<'alloc>) {
        ...
        match ast {
            ClassElement::MethodDefinition {
                is_static, method, ..
            } => self.visit_enum_class_element_variant_method_definition(is_static, method),
            ClassElement::FieldDefinition { name, init, .. } => {
                self.visit_enum_class_element_variant_field_definition(name, init)
            }
        }
    ...
    fn visit_enum_class_element_variant_method_definition(
        ...
        self.enter_enum_class_element_variant_method_definition(is_static, method);
        self.visit_method_definition(method);
        self.leave_enum_class_element_variant_method_definition(is_static, method);
    }
    ...
    fn visit_enum_class_element_variant_field_definition(
        ...
        self.enter_enum_class_element_variant_field_definition(name, init);
        self.visit_class_element_name(name);
        if let Some(item) = init {
            self.visit_expression(item);
        }
        self.leave_enum_class_element_variant_field_definition(name, init);
    }
    ...
    fn visit_module_items(&mut self, ast: &mut ModuleItems<'alloc>) {
        ...
        match ast {
            ...
            ModuleItems::Statement(ast) => self.visit_enum_module_items_variant_statement(ast),
        }
    ...
    fn visit_export(&mut self, ast: &mut Export<'alloc>) {
        ...
        match ast {
            ...
            Export::ClassDeclaration(ast) => self.visit_enum_export_variant_class_declaration(ast),
    ...
    fn visit_enum_export_variant_function_declaration(&mut self, ast: &mut Function<'alloc>) {
        self.enter_enum_export_variant_function_declaration(ast);
        self.visit_function(ast);
        self.leave_enum_export_variant_function_declaration(ast);
    }

    fn enter_enum_export_variant_function_declaration(&mut self, ast: &mut Function<'alloc>) {}

    fn leave_enum_export_variant_function_declaration(&mut self, ast: &mut Function<'alloc>) {}

    fn visit_enum_export_variant_class_declaration(&mut self, ast: &mut ClassDeclaration<'alloc>) {
        self.enter_enum_export_variant_class_declaration(ast);
        self.visit_class_declaration(ast);
        self.leave_enum_export_variant_class_declaration(ast);
    }

    fn enter_enum_export_variant_class_declaration(&mut self, ast: &mut ClassDeclaration<'alloc>) {}

    fn leave_enum_export_variant_class_declaration(&mut self, ast: &mut ClassDeclaration<'alloc>) {}
    ...
    fn visit_variable_declaration_or_assignment_target(
        ...
        match ast {
            VariableDeclarationOrAssignmentTarget::VariableDeclaration(ast) => self
                .visit_enum_variable_declaration_or_assignment_target_variant_variable_declaration(
                    ast,
                ),
            VariableDeclarationOrAssignmentTarget::AssignmentTarget(ast) => self
    ...
    fn enter_variable_declaration_or_assignment_target(
        &mut self,
        ast: &mut VariableDeclarationOrAssignmentTarget<'alloc>,
    ) {
    }

    fn leave_variable_declaration_or_assignment_target(
        &mut self,
        ast: &mut VariableDeclarationOrAssignmentTarget<'alloc>,
    ) {
    }
    ...
    fn enter_variable_declaration_or_expression(
        &mut self,
        ast: &mut VariableDeclarationOrExpression<'alloc>,
    ) {
    }

    fn leave_variable_declaration_or_expression(
        &mut self,
        ast: &mut VariableDeclarationOrExpression<'alloc>,
    ) {
    }
    ...
    fn visit_cover_parenthesized(&mut self, ast: &mut CoverParenthesized<'alloc>) {
        ...
        match ast {
            CoverParenthesized::Expression { expression, .. } => {
                self.visit_enum_cover_parenthesized_variant_expression(expression)
            }
    ...
    fn visit_enum_cover_parenthesized_variant_expression(
        ...
        self.enter_enum_cover_parenthesized_variant_expression(expression);
        self.visit_expression(expression);
        self.leave_enum_cover_parenthesized_variant_expression(expression);
    }
