...
use stencil::function::{
    FunctionFlags, FunctionKind, FunctionStencil, FunctionStencilIndex, FunctionStencilList,
    SourceExtent,
};
use stencil::scope::{
            ...
            for fun in functions {
                ...
                match scope {
                    ScopeData::Lexical(data) => {
                        data.mark_annex_b_function(fun.binding_index.into());
                    }
...
pub struct FunctionStencilBuilder {
    ...
    ///
    /// It's separated into `function_stencil_indices` and `functions`,
    /// because `FunctionStencil` is referred from multiple places:
    ...
    /// The innermost one is the current function that inner function is going
    /// to be stored into.
...
impl FunctionStencilBuilder {
        ...
        T: SourceLocationAccessor + NodeTypeIdAccessor,
    {
        ...
        let script = ScriptStencilBase::lazy_function(is_generator, is_async);
        let flags = FunctionFlags::interpreted(kind, is_lambda, is_generator, is_async);
